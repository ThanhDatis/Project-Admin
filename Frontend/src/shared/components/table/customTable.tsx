...
export default function CustomTable<T extends Record<string, any>>({
  ...
  const [SelectedIds, setSelectedIds] = useState<GridRowSelectionModel | undefined>(undefined);
  ...
  function handleSortModelChange(_model: GridSortModel, _details: GridCallbackDetails<any>): void {
  ...
  return (
    ...
    >
      <DataGrid
        ...
        getRowClassName={className ? (params) => className(params.row) : undefined}
        localeText={{
          footerRowSelected: (count) => (checkboxSelection ? `Selected ${count} rows` : ''),
          noRowsLabel: noDataMessage,
          paginationDisplayedRows: ({ from, to, count }) =>
            `${from}–${to} of ${count !== -1 ? count : '0'}`,
        }}
        slots={{
          noRowsOverlay: () => <tableNoData.TableNoData message={noDataMessage} />,
          noResultsOverlay: () => <tableNoData.TableNoData message={noDataMessage} />,
        }}
        slotProps={{
